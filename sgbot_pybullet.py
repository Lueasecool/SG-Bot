...
def main():
   # rearrange_dataset_base = '/media/ymxlzgy/Data/Dataset/sgbot_dataset'
    rearrange_dataset_base = '/remote-home/2332082/data/sgbot_dataset'
    ...
    stats_dataloader = torch.utils.data.DataLoader(
        stats_dataset,
        batch_size=1,
        collate_fn=collate_fn_vaegan,   #collate_fn为用于设置处理数据格式的函数
        shuffle=False,
        num_workers=0)
    ...
    #pick_place_pair_list：目标场景路径和原始场景路径组成的元组
    ...
    for id in range(0,len(pick_place_pair_list)):
        ...
        result_dir = os.path.join(test_save_path, result_name)#设置保存路径，./sg_bot_{mode}_results/...type-X-in-X
        os.makedirs(result_dir, exist_ok=True)
        shutil.copy(pick_place_pair_list[id][0].replace('json','png'), result_dir)#将json对应的 PNG 文件复制到  result_dir  目录中
        shutil.copy(pick_place_pair_list[id][1].replace('json', 'png'), result_dir)


        load_preprocessed = 1

        # generate a goal graph   ！！！这段函数不明所以
        ...
        bboxes_dict, points_dict = post_processing(bboxes, points, data, env.table_height)#因为桌子提高坐标
