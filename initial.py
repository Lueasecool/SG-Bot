...
class RearrangeEnv():
    ...
    def reset_rearrange(self, id, use_ur5=True):
        ...
        '''加载初始场景下的机械臂、目标物体位姿，以及保存目标场景下的物体位姿和包围盒信息'''
        ...
        if self.use_ur5:
            ...
            self.ee_to_tip = np.dot(np.linalg.inv(tip_transform_mat), ee_transform_mat)#末端到夹爪的变换矩阵
        ...
        with open(pick_info) as f:
            self.source_info = json.load(f)#初始场景下的json
        with open(place_info) as f:
            self.target_info = json.load(f)#goal场景下的json

        self.cam_info = self.source_info['camera_data']
        z_support_max = 0
        for source in self.source_info['objects']:
            class_name = source['class']#  eg:teapot  ,bowl
            object_name = source['name']#eg: teapot1,  bowl_9
            ...
            ini_pose = np.array(source['local_to_world_matrix']).reshape(4, 4).T#在世界坐标系下的位姿
            ini_bbox = np.array(source['param6'])#包围盒，[l, w, h, cx, cy, cz]

            # normalize scale sR /= s  s = Frobenius
            rot_F = np.linalg.norm(ini_pose[:3, :3], ord='fro')
            ini_pose[:3, :3] /= (rot_F / np.sqrt(3))#对旋转进行归一化
